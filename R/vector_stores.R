...
vector_stores <- R6Class(
  ...
  public = list(
    ...
    file_batche_retrieve=function(vector_store_id,batch_id,verbosity=0){
      result <- private$api_call("vector_stores", paste0("/", vector_store_id,"/files/",batch_id), method = "GET",headers = list(`Content-Type` = "application/json",`OpenAI-Beta`="assistants=v2"), verbosity = verbosity)
      if (inherits(result, "openai_error")) {
        return(list(success=FALSE, message=result$get_message(), type=result$get_type()))
      }else{
        return(result$data)
    ...
    file_batche_cancel=function(vector_store_id,batch_id,verbosity=0){
      result <- private$api_call("vector_stores", paste0("/", vector_store_id,"/file_batches/",batch_id,"/files"), method = "POST",headers = list(`Content-Type` = "application/json",`OpenAI-Beta`="assistants=v2"), verbosity = verbosity)
      if (inherits(result, "openai_error")) {
        return(list(success=FALSE, message=result$get_message(), type=result$get_type()))
      }else{
        return(result$data)
